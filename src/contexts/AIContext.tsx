...
export function AIProvider({ children }: { children: ReactNode }) {
  ...
  const sendMessage = async (message: string, patientContext?: any): Promise<AIMessage> => {
    ...
    try {
      ...
      const response = await deepSeekService.chat(message, patientContext);
  ...
  const loadConversations = async (patientId: string) => {
    try {
      const conversations = await deepSeekService.getConversations(patientId);
  ...
  const getRecommendation = async (type: string, context: any): Promise<AIRecommendation> => {
    dispatch({ type: 'SET_PROCESSING', payload: true });
    try {
      const recommendation = await deepSeekService.getRecommendation(type, context);
  ...
  const analyzeSymptoms = async (symptoms: any[], patientContext: any) => {
    dispatch({ type: 'SET_PROCESSING', payload: true });
    try {
      const analysis = await deepSeekService.analyzeSymptoms(symptoms, patientContext);
