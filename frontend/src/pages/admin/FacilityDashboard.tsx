...
import { Alert, Box, Button, Card, CardContent, Dialog, DialogActions, DialogContent, DialogTitle, Snackbar, Stack, Typography } from '@mui/material';
...
export const FacilityDashboard: React.FC = () => {
  ...
  return (
    <Box sx={{ px: { xs: 1, md: 2 }, pb: 4 }}>
      <Typography variant="h5" fontWeight={800} sx={{ mb: 0.5 }}>Facility Admin Dashboard</Typography>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>{user?.name} (Facility Admin)</Typography>
      ...
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} sx={{ mb: 3 }}>
        <Card sx={{ flex: 1 }}><CardContent><Typography variant="caption">Total Doctors</Typography><Typography variant="h5">{stats.totalDoctors}</Typography></CardContent></Card>
        <Card sx={{ flex: 1 }}><CardContent><Typography variant="caption">Total Nurses</Typography><Typography variant="h5">{stats.totalNurses}</Typography></CardContent></Card>
        <Card sx={{ flex: 1 }}><CardContent><Typography variant="caption">Total Registered Patients</Typography><Typography variant="h5">{stats.totalPatients}</Typography></CardContent></Card>
        <Card sx={{ flex: 1 }}><CardContent><Typography variant="caption">Today's Visits</Typography><Typography variant="h5">{stats.todaysVisits}</Typography></CardContent></Card>
      </Stack>

      <Stack spacing={3}>
        <Card><CardContent><Typography variant="h6" gutterBottom>Create Doctor</Typography><CreateDoctorForm loading={loading} onSubmit={createDoctor} /></CardContent></Card>
        <Card><CardContent><Typography variant="h6" gutterBottom>Create Nurse</Typography><CreateNurseForm loading={loading} onSubmit={createNurse} /></CardContent></Card>
        <Card><CardContent><Typography variant="h6" gutterBottom>Register Patient (Facility Admin Only)</Typography><RegisterPatientForm loading={loading} onCheckDuplicates={checkDuplicate} onSubmit={registerPatient} /></CardContent></Card>
        <Card><CardContent><Typography variant="h6" gutterBottom>Doctors List</Typography><DoctorsList rows={doctors} /></CardContent></Card>
        <Card><CardContent><Typography variant="h6" gutterBottom>Nurses List</Typography><NursesList rows={nurses} /></CardContent></Card>
        <Card><CardContent><Typography variant="h6" gutterBottom>Patients List (Basic Info Only)</Typography><PatientsList rows={patients} /></CardContent></Card>
        <Card>
          <CardContent>
