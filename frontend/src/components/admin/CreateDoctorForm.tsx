  ...
  return (
    <Box component="form" onSubmit={submit}>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} md={6}>
          <TextField select fullWidth required label={bi('Specialization', 'ስፔሻላይዜሽን')} value={form.specialization} onChange={(e) => setField('specialization', e.target.value)}>
      ...
      <Button sx={{ mt: 2 }} type="submit" variant="contained" disabled={loading}>{loading ? bi('Creating...', 'በመፍጠር ላይ...') : bi('Create Doctor', 'ሐኪም ፍጠር')}</Button>
    </Box>
  );
};
