  ...
  return (
    <Box component="form" onSubmit={submit}>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} md={6}>
          <TextField select fullWidth required label={t('Specialization', 'ስፔሻላይዜሽን')} value={form.specialization} onChange={(e) => setField('specialization', e.target.value)}>
      ...
      <Button sx={{ mt: 2 }} type="submit" variant="contained" disabled={loading}>{loading ? t('Creating...', 'በመፍጠር ላይ...') : t('Create Doctor', 'ሐኪም ፍጠር')}</Button>
    </Box>
  );
};
